//commented stdafx, conio(getch), QueryPerformance... with tstart
//#include "stdafx.h" 
#include <time.h>
//#include <conio.h>
#include <stdlib.h>
...
void MultiDelimiterCodes::decodeFib() {
	FibT x={3,71,11,4,121,19};
uint j=0,index,i=0;
...
int MultiDelimiterCodes::get_size(unsigned int x) {
int s=0;
	while(x>0) {
		x>>=1;
		s++;
...
void MultiDelimiterCodes::insert_1(unsigned int& x,int pos) {
...
int MultiDelimiterCodes::d235_length(uint x) {
uint k;
...
unsigned int MultiDelimiterCodes::get_code(unsigned int x) {
...
void MultiDelimiterCodes::flush_to_byte(unsigned int x) {
int k(0),p;
...
int k(0),p;
unsigned int j;
//find the leftmost 1. Codeword starts on 1 bit left of it
...
uint MultiDelimiterCodes::invert_d235(uint x) {
uint l,i;
uint res=0;
...
void bn::prepend(const char* str) {
int k=strlen(str);
	for(int j=k-1;j>=0;j--) {
		if(str[j]=='1')
			x|=(1<<last_bit);
...
void bn::add(const char* str, int s) {
int k=strlen(str);
	for(int j=0;j<s;j++) {
		x<<=k;
		last_bit+=k;
...
int MultiDelimiterCodes::cmp_bit(bn x,const char u) {
...
int MultiDelimiterCodes::cmp_(bn x,const char* str) {
int k=strlen(str),i;
	if(k>1)
		if(str[1]=='s') {
...
bool MultiDelimiterCodes::cmp_rest(bn x,const char* str) {
int k=strlen(str),i;
	if(str[k-1]=='s')
		k--;
	for(i=0;i<k && x.last_bit>0;i++,x.last_bit--)
...
int MultiDelimiterCodes::fw(bn& x,bn& res,bool fin_flag) {
	if(x.last_bit<=0)
		return 2;
...
int MultiDelimiterCodes::get_w1(uint nevr,bn& x,bn& y) { // x - input byte, y - output w1
int s;
	switch(nevr) {
...
void MultiDelimiterCodes::print_bin_char(uchar c) {
	for(uchar i=128;i;i>>=1)
		printf("%d",c&i?1:0);
	printf(" ");
}

int MultiDelimiterCodes::write_table0(bn w1,bn nv) {
uint x=(1<<31);              //f1=1
...
int MultiDelimiterCodes::write_table1(bn w1,bn w2,bn nv) {
uint x=(1<<30);
	x|=(w1.x<<(16-w1.last_bit));
	x|=16-w1.last_bit;
	x|=(w2.x<<16);
...
int MultiDelimiterCodes::write_table2(bn w1,bn w2,bn w3,bn nv) {
uint x=0;
	x|=(w1.x<<(16-w1.last_bit));
	x|=16-w1.last_bit;
	x|=(w2.x<<16);
...
int MultiDelimiterCodes::write_table01(bn w1,bn nv,tabs* v) {
uint x=(1<<31);              //f1=1
	v->f=0;
	x|=(w1.x<<(16-w1.last_bit));
	x|=w1.last_bit;
...
int MultiDelimiterCodes::write_table11(bn w1,bn w2,bn nv,tabs* v) {
uint x=(1<<30);
	v->f=1;
	x|=(w1.x<<(16-w1.last_bit));
	x|=16-w1.last_bit;
...
int MultiDelimiterCodes::write_table21(bn w1,bn w2,bn w3,bn nv,tabs* v) {
uint x=0;
	x|=(w1.x<<(16-w1.last_bit));
	x|=16-w1.last_bit;
	x|=(w2.x<<16);
...
void MultiDelimiterCodes::initTAB1() {
uint i,j;
int fin,mv3=0;
bn x,w1,w2,w3,nv;
	for(i=0;i<10;i++) {
...
void MultiDelimiterCodes::initTAB() {
uint i,j;
int fin,mv3=0;
bn x,w1,w2,w3,nv;
	for(i=0;i<10;i++) {
...
int MultiDelimiterCodes::getIndex(int n, int l) {
    int ans = start[l];
    for (int i = l - 1; i >= 0; i--)
        if (n & 1 << i)
            ans += fib3[l - 1 - i];
...
int MultiDelimiterCodes::f2(int n,int l) {
    if (l < 0)
        return M - l;
    return getIndex(n, l) - start[l];
}

entry** MultiDelimiterCodes::build_partial_decoding_table() {
...
int** MultiDelimiterCodes::build_Fib3() {
...
void MultiDelimiterCodes::full_Fib3_decode() {
	int **Fib3 = build_Fib3();
	...
	//QueryPerformanceCounter(&tstart); // commented
	...
	//QueryPerformanceCounter(&tend);//commented
...
void MultiDelimiterCodes::dic_to_file() {
fstream out;
...
int MultiDelimiterCodes::scdc_encode(int L,uint* numbers,uchar* outp) {
int i,cur,x,j=0;
uchar temp[10];
	for(i=0;i<L;i++) {
		cur=9;
...
int MultiDelimiterCodes::rpbc_encode(int L,uint* numbers,uchar* outp) {
int i,cur,x,j=0;
	for(i=0;i<L;i++) {
		x=numbers[i];
		if(x<v1)
...
#pragma optimize( "", off )
int MultiDelimiterCodes::scdc_decode(uchar* code, uint* out,int L_coded) {
...
#pragma optimize( "", off )
void MultiDelimiterCodes::set_v1_v4() {
	v1=220;v2=35;v3=1;v4=0;
}

void MultiDelimiterCodes::create_tables() {
	int start=0;
	set_v1_v4();
...
int MultiDelimiterCodes::rpbc_decode(uchar* code, uint* out,int L_coded) {
	int b,k=0,offset;
...
void MultiDelimiterCodes::print_codeword(uint x) {
	uint k,p;
...
void MultiDelimiterCodes::obtain_codes235() {
	const int L=120000;
...
#pragma optimize( "", off )
int MultiDelimiterCodes::decode_C() {
	uint w,r,k,i;
	tabs *v;
...
void MultiDelimiterCodes::dict_from_file() {
	int i=0;
...
void MultiDelimiterCodes::dict_from_file_scdc() {
	int i=0;
...
bool MultiDelimiterCodes::d235_in_i235(uint x) {
	int i;
...
void MultiDelimiterCodes::d235_codeword_map(int L) { //generates map <n,<code(n),length(code(n))>>
	uint x,k(0),p;
...
ai_state MultiDelimiterCodes::a_step(ai_state prev,int bit) {
...
ai_state MultiDelimiterCodes::a_step1(ai_state prev,int bit) {
	ai_state r={0,prev.L+1,prev.p,-1};
...
void MultiDelimiterCodes::decodeI235(int N) {
...
void MultiDelimiterCodes::decodeI235_1(int N) {
	ai_state state={0,1,-1,-1};
...
void MultiDelimiterCodes::formT235_inner(int L,ai_state state) {
	tabsi235 v;
	ai_state a=state;
		...
		if(v.p1==-1)
			...
			if(v.p2==-1)
				...
				if(v.p3==-1)
					...
					v.p4=state.p;
		/*v.al=(tabsi235*)(T235+256*state_max*state.L+256*state.st);
		*(T235+256*state_max*a.L+256*a.st+x)=v;*/
		v.al=(tabsi235*)(T235+256*Lmax*state.st+256*state.L);
...
uint MultiDelimiterCodes::pack(tabsi2351 v, ai_state s) {
...
void MultiDelimiterCodes::formT235_inner1(int L,ai_state state) {
	tabsi2351 v;
	ai_state a=state;
	...
	for(uint x=0;x<256;x++) {
		...
		/*if(v.p3>0)
			cout << v.p2 <<" "<< v.p3 << " | ";*/
		v.al=(tabsi2351*)(T235_1+256*state_max*state.L+256*state.st);
		*(T235_1+256*state_max*a.L+256*a.st+x)=v;
		/*v.al=(tabsi2351*)(T235_1+256*Lmax*state.st+256*state.L);
...
void MultiDelimiterCodes::formT2351() {
	state_max=3;
...
void MultiDelimiterCodes::formT235() {
	for(int a=0;a<state_max;a++)	
		for(int L=0;L<Lmax;L++) {
			ai_state state={a,L,0,-1};
...
void MultiDelimiterCodes::f1() {
	glob_++;
	out_i235[glob_]=Dict_i235+113;
}

void MultiDelimiterCodes::f2() {
	glob_++;
	out_i235[glob_]=Dict_i235+113;
}
...
int MultiDelimiterCodes::decode_i235_fast_orig() {
	uint p=0,k=0,i=0,temp=0,x,y,numb=256,p1,p2,p3,p4;
	uint real_len;
	tabsi235 t={0,0,0,0,( tabsi235*) T235+256},*v1;
	...
	for(i=0;i<ilen;i++) {	//21
		//actions[i&1]();
		//r1++;
		//suml+=(t.al-T235)/sizeof(tabsi235);
...
int MultiDelimiterCodes::decode_i235_fast() {
	uint p=0,k=0,i=0,temp=0,x,y,numb=256,p1,p2,p3,p4,tt,cds,ptr=11;
	tabsi2351 t={0,0,0,0,0,(tabsi2351*) T235_1+256*11},*v1;
...
void MultiDelimiterCodes::gen_reverse() {
...
inline void addWord(std::map<std::string, int>& i235_map, const std::string& word){
...
void MultiDelimiterCodes::write_to_file_i235(int N,string** indices,char* fname) {
	string word;	
...
void MultiDelimiterCodes::i235_test(int N,char* fname) {
	string word;
...
void MultiDelimiterCodes::write_to_file_binary(uchar* code,const char* filename,int N) {
	string word;	
